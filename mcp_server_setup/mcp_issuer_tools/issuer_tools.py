...
VC_PATH = REPO_ROOT / "domain_b" / "agents" / "agent_y" / "agent_y_god_vc.json"
KEY_PATH = REPO_ROOT / "domain_b" / "agents" / "agent_y" / "agent_y_key.json"
...
# def check_signature_vp(vp: Union[str, dict]) -> bool:
    """
    ...
    """
    #     # Manchmal wird ein bereits serialisiertes JSON nochmal als String übergeben → zweimal laden
    # try:
    #     # Erste Ebene laden
    #     vp_data = json.loads(vp)
    #     # Wenn das Ergebnis immer noch ein String ist, nochmal parsen
    #     if isinstance(vp_data, str):
    #         vp_data = json.loads(vp_data)
    ...
    #             vp_data = json.loads(vp_data)
    # except json.JSONDecodeError as e:
    #     print(f"[ERROR] Cannot decode VP: {e}")
    #     return False
...
class IssueVCArgs(BaseModel):
    ...
    credential_application: Dict = Field(
        ...,
        description="The credential application from Agent B."
    )

# @tool("issue_credential", args_schema=IssueVCArgs)
def issue_credential(credential_application: Dict) -> str:
    """
    Issue a Verifiable Credential to Agent B based on their credential application.
    Args:
        credential_application (Dict): The credential application from Agent B.
    Returns:
        str: The signed Verifiable Credential as a JSON string.
    """
    agent_seed, issuer_did = load_seed()
    
    # Extract Agent B's DID from the credential application
